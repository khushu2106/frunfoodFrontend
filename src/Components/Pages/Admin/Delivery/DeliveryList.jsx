...
const DeliveryList = () => {
    ...
    const fetchDeliveryBoys = async () => {
        try {
            ...
            const res = await axios.get(
                "http://localhost:5000/api/delivery-boy/all-delivery-boys"
            );

            // ✅ Important: correct data path
    ...
    <tr key={boy.delivery_id || index}>
        <td style={styles.td}>{boy.delivery_id}</td>
        <td style={styles.td}>{boy.role_id}</td>
        <td style={styles.td}>
            {boy.availability === "YES" ? "🟢 Available" : "🔴 Not Available"}
        </td>
        <td style={styles.td}>{boy.vehicle_no}</td>
    </tr>
))
